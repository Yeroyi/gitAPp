...
public class FaultActivity extends BaseActivity implements AMap.InfoWindowAdapter, AMap.OnMapClickListener, AMap.OnMarkerClickListener, AMap.OnInfoWindowClickListener, RouteSearch.OnRouteSearchListener, ShareSearch.OnShareSearchListener, AMapLocationListener {
...
//        device_name.setText(deviceName);
        ...
        if (intent != null) {
            ...
            if (data != null && !data.isEmpty()) {
                ...
                fragmentbutton.setTextColor(Color.BLACK);
                if (fireOrFaultName.equals("DetectorLossFault")) {
                    wave_bg.setBackgroundColor(Color.YELLOW);
                    fragmentbutton.setText("丢失故障");
                    fragmentbutton.setBackgroundResource(R.drawable.shape_circle_yellow);
                    callPhone.setVisibility(View.INVISIBLE);
                } else if (fireOrFaultName.equals("UndervoltageFault")) {
                    wave_bg.setBackgroundColor(Color.YELLOW);
                    if (productName.equals("燃气报警器")) {
                        fragmentbutton.setText("设备故障");
                    } else {
                        fragmentbutton.setText("欠压故障");
                    }
                    fragmentbutton.setBackgroundResource(R.drawable.shape_circle_yellow);
                    callPhone.setVisibility(View.INVISIBLE);
                } else if (fireOrFaultName.equals("PollutionFault")) {
                    wave_bg.setBackgroundColor(Color.YELLOW);
                    fragmentbutton.setText("污染故障");
                    fragmentbutton.setBackgroundResource(R.drawable.shape_circle_yellow);
                    callPhone.setVisibility(View.INVISIBLE);
                }else if (fireOrFaultName.equals("StaticPointFault")) {
                    wave_bg.setBackgroundColor(Color.YELLOW);
                    fragmentbutton.setText("静态点故障");
                    fragmentbutton.setBackgroundResource(R.drawable.shape_circle_yellow);
                    callPhone.setVisibility(View.INVISIBLE);
                }
                device_name.setText(deviceName);
...
//        aMap.setLocationSource(this);// 设置定位监听
    ...
    protected void onDestroy() {
        ...
        mMapView.onDestroy();
        FaultMessage pushMessage = new FaultMessage();
